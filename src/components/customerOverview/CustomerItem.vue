  ...
  <div
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
      <div class="mb-4 md:mb-0">
        ...
        <p class="text-sm text-gray-600">
          <strong>Telefonnummer: </strong> {{ customer.telefonnummer || '---' }}
        </p>
        <p class="text-sm text-gray-600">
        ...
        <p class="text-sm text-gray-600">
          ...
          <span v-if="customerDogs.length">
            <span v-for="chunk in chunkedDogs" :key="chunk[0].id" class="block">
              <span
                v-for="dog in chunk"
                :key="dog.id"
                class="mr-2 text-blue-900 hover:underline cursor-help"
                @click.stop="viewCustomerDogs">
                {{ dog.name }}
              </span>
            </span>
          </span>
      ...
      <!-- Action Buttons -->
      <div
        class="flex space-x-4 mt-4 md:mt-0 justify-end w-full md:w-auto">
        <button
    ...
    <div v-if="showDetails" class="text-gray-700 mt-4 space-y-2">
      ...
      <p class="text-sm text-gray-600">
        <strong>Straße: </strong> {{ customer.straße || '---' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>PLZ: </strong> {{ customer.plz || '---' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>Ort: </strong> {{ customer.ort || '---' }}
      </p>

      <h3 class="text-lg font-semibold text-gray-800 mt-4">Persönliche Informationen</h3>
      <p class="text-sm text-gray-600">
        <strong>Anrede: </strong> {{ customer.anrede || '---' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>Ausweisnummer: </strong> {{ customer.ausweisnr || '---' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>Geburtsdatum: </strong>
        {{ customer.geburtsdatum ? new Date(customer.geburtsdatum).toLocaleDateString() : '---' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>Geburtsort: </strong> {{ customer.geburtsort || '---' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>Sprache:</strong> {{ customer.sprache || '---' }}
      </p>

      <h3 class="text-lg font-semibold text-gray-800 mt-4">Sonstiges</h3>
      <p class="text-sm text-gray-600">
        <strong>AGBs Akzeptiert:</strong> {{ customer.agbsAkzeptiert ? 'Ja' : 'Nein' }}
      </p>
      <p class="text-sm text-gray-600">
        <strong>Keine MWST:</strong> {{ customer.noMWST ? 'Ja' : 'Nein' }}
      </p>
    </div>
    <EditCustomerOverlay
