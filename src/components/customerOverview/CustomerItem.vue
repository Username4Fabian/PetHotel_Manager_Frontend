...
const deleteCustomer = async () => {
  if (window.confirm(`Sind Sie sicher, dass Sie den Kunden: ${props.customer.firstName} ${props.customer.lastName} löschen möchten?`)) {
    try {
    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex space-x-2">
        <button
          ...
          class="text-gray-500 hover:text-red-600 transition-colors duration-200 text-3xl hover:cursor-pointer">
          <i class="fas fa-trash-alt"></i>
        </button>
        <button
          @click.stop="showEditOverlay = true"
          class="text-gray-500 hover:text-blue-600 transition-colors duration-200 text-3xl hover:cursor-pointer">
